...
The real-application trial added a Unix requirement that the fixture suite had missed: some TUIs open `/dev/tty` instead of using only inherited standard streams. Current source starts the target in a new session and assigns the PTY slave as its controlling terminal. The published `v0.1.0-rc.1` Linux asset predates that fix. The replacement `v0.1.0-rc.2` Linux and macOS archives passed the real `/dev/tty` package gate and the downloaded-asset install smoke.
