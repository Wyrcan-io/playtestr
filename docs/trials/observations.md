...
| R3-T10 | K9s/Windows rc.2 | Verify a Unicode live field independently after a screen pass. | Trial harness portability | Windows PowerShell 5 misdecoded a literal lambda in the oracle script and falsely rejected a correct screen and API value. | The failed oracle attempt is retained. Constructing the expected value with code point `U+03BB` passed against the unchanged live ConfigMap. | Construct non-ASCII oracle constants from code points or read them from a verified UTF-8 input under legacy PowerShell. | One Windows harness attempt. | Keep oracle encoding explicit; no runner change required. | Trial guidance | Resolved in candidate harness |
| R3-T11 | K9s/Windows and Linux rc.2 | Prove a controlled pod deletion replaced exactly the intended pod. | Trial oracle timing | An immediate exactly-one-pod check can reject a healthy Deployment while the old pod is terminating beside the new pod. | The first Windows delete runner passed, then the API briefly returned two UIDs. A bounded convergence oracle passed on Windows recovery and Linux by requiring one new Ready UID, no deletion timestamp, and absence of the old UID. | Poll the exact label selector to convergence and validate UIDs; never treat transient item count as the final postcondition. | Reproduced on one host and prevented on the second. | Add convergence semantics to the K9s recipe. | Trial guidance | Resolved in candidate harness |
