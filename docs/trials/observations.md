...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| R3-T01 | Lazygit/Linux | Launch a full-screen TUI that opens `/dev/tty`. | Runner correctness | Published Linux candidate exits before rendering. | `v0.1.0-rc.1` returned `unexpected_exit`; a Unix real-PTY regression failed before `Setctty` and passed after it. | Use a checkout-built runner containing the controlling-terminal fix. | One target, then validated with Lazygit and K9s. | Release a new candidate and rerun its downloaded Linux asset. | Release candidate | Fixed locally; publication pending |
