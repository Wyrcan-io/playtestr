...
R3c's technical entry gate is accepted for the exact `v0.1.0-rc.2` bytes and tested host labels above. R4 still owns the public-contract audit, maintenance/release documentation, stable-version native builds, stable archive checksums, public installation verification, and rerunning these nine primary workflows against the actual stable bytes. A behavioral code change requires a new candidate and affected reruns. Independent review, voluntary reuse, and participant-owned CI remain open under R3 after publication.
